...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->linkRegister->Text = L"Реєстрація";
...
#pragma endregion
	...
	private: System::Void btOK_Click(System::Object^ sender, System::EventArgs^ e) {
		// Отримуємо ім'я користувача та пароль з текстбоксів
		std::string username = msclr::interop::marshal_as<std::string>(tbUsername->Text);
		std::string password = msclr::interop::marshal_as<std::string>(tbPassword->Text);

		// Відкриваємо файл для читання
		std::ifstream infile("User.txt");

		// Читаємо файл по рядках та порівнюємо ім'я користувача та пароль з записами в файлі
		std::string line;
		...
		// Виводимо відповідне повідомлення про успішну або невдалу авторизацію
		if (authenticated) {
